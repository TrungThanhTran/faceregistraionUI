...
def match(recognizer, feature1, dictionary):
    ...
    if max_score < COSINE_THRESHOLD:
        return False, ("", 0.0)
    print(
        f'confidence score = {(math.pi - math.acos(max_score)) * 100 / (math.pi)} %')
...
def crop_face(image, face, file_name):
    ...
    width = int(face[2])
    height = int(face[3])
    cv2.rectangle(image, (x_coor, y_coor), (x_coor + width, y_coor + height), (0, 255, 0), 2)
    roi_color = image[y_coor:y_coor + height, x_coor:x_coor + width]
    if file_name is not None:
        cv2.imwrite('data/temp/' + file_name + '_face.png', roi_color)
    return roi_color

def analyze_faces(face):
    confidence = face[-1]
    landmarks = np.reshape(face[4:-1], (5, 2))
    locations = list(map(int, face[:4]))
    return landmarks, locations, confidence
